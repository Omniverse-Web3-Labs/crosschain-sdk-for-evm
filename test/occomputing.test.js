...
const CrossChain = artifacts.require('@omniverselabs/contracts/TwoPhaseCommitCrossChain');
const MessageVerify = artifacts.require('@omniverselabs/contracts/MessageVerify');
...
contract('OCComputing', function(accounts) {
    ...
    let initContract = async function() {
        ...
        // register target
        await ocComputing.registerDestnContract('receiveComputeTask', 'BSCTEST', OCComputing.address, 'receiveComputeTask');
        await ocComputing.registerDestnContract('receiveComputeTaskCallback', 'BSCTEST', OCComputing.address, 'receiveComputeTaskCallback');
    }
    ...
    describe('Receive OC Computing Task', function() {
        it('should execute successfully', async () => {
            ...
            let argument = [1, 'BSCTEST', OCComputing.address, owner, [0], to, action, calldata, [0, utils.toHexString(utils.stringToByteArray('0x11111111'))], 0];
    ...
    describe('Receive OC Computing Task Callback', function() {
        it('should execute successfully', async () => {
            ...
            let calldata = {items: [item]};
            let argument = [2, 'BSCTEST', OCComputing.address, owner, [0], to, action, calldata, [1, '0x11111111'], 0];
            await crossChain.receiveMessage(argument, {from: user1});
